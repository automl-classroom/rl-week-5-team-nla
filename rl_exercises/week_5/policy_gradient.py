...
class REINFORCEAgent(AbstractAgent):
    ...
    def predict_action(
        ...
        if evaluate:
            action = int(torch.argmax(action_probabilities, dim=-1).item())
            return action, {}
        else:
            dist = torch.distributions.Categorical(action_probabilities)
            action = dist.sample()
            log_probs = dist.log_prob(action)
            return int(action.item()), {"log_prob": log_probs}
    ...
    def update_agent(
        ...
        # When not adding unbiased=False, the code failed to pass one of the tests,
        # so this was added based on the suggestion from dear ChatGPT. =))
    ...
    def evaluate(
        ...
        return float(returns_array.mean()), float(returns_array.std())
